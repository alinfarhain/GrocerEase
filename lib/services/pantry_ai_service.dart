...
class PantryAiService {
  ...
  Future<List<DetectedPantryItem>> detectItemsFromImage(File imageFile) async {
    ...
    final response = await _httpClient.post(
      // ← now calls swift-action instead of detect-pantry-items
      Uri.parse('$_supabaseUrl/functions/v1/swift-action'),
      headers: {
      ...
      body: jsonEncode({
        ...
        'mode': 'pantry_scan',   // ← new mode in swift-action
      }),
    );
    ...
    // pantry_scan returns { items: [...] }
    // other modes return  { result: ... }
    // handle both just in case
    final List<dynamic> items =
        data['items'] ?? (data['result'] is List ? data['result'] : []);
